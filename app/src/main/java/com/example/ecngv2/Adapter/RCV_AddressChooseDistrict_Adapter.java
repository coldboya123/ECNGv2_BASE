...
public class RCV_AddressChooseDistrict_Adapter extends RecyclerView.Adapter<RCV_AddressChooseDistrict_Adapter.ViewHolder> {
    ...
    public RCV_AddressChooseDistrict_Adapter(Context context, List<District> list, IAddUserAddress iAddUserAddress) {
    ...
    @NonNull
    @Override
    public RCV_AddressChooseDistrict_Adapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.custom_rcv_chooseaddress, parent, false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull RCV_AddressChooseDistrict_Adapter.ViewHolder holder, int position) {
        holder.txt.setText(list.get(position).getName());
        holder.txt.setOnClickListener(v -> iAddUserAddress.OnSelectedDistrict(list.get(position)));
    }
